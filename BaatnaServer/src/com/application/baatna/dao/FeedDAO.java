...
public class FeedDAO {
	...
	/**
	 * Get feed items of user's which have recently joined and are near to the provided user
	 * */
	public ArrayList<NewsFeed> getNearbyUsers(Location location) {
		...
		try {
			...
			String sql = "SELECT * FROM NEWSFEED WHERE TYPE = 1";//append params for nearby filter.
	...
	public ArrayList<NewsFeed> getUsersWithWishes(Location location) {
		...
		try {
			...
			String sql = "SELECT * FROM NEWSFEED WHERE TYPE = 2";//append params for nearby filter.
			SQLQuery query = session.createSQLQuery(sql);
			query.addEntity(NewsFeed.class);
			java.util.List results = (java.util.List) query.list();
	 ...
	 * Session tables.
	 * @param 
	 * type 1: userIdFirst joined, 
	 * type 2: userIdFirst requested wishId, 
	 * type 3: userIdFirst gave wishId to userIdSecond
	 *    
	 * */
	public boolean addFeedItem(int type, long timestamp,
			int userIdFirst, int userIdSecond, int wishId) {
		NewsFeed feedItem;
		Session session = null;
		try {
			session = DBUtil.getSessionFactory().openSession();
