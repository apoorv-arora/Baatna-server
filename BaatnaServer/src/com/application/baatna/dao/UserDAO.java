...
public class UserDAO {
	...
	public ArrayList<com.application.baatna.bean.Session> getUsersNotEquals(int userId) {
		...
		try {
			...
			if (userSession != null && userSession.getLocation() != null && userSession.getLocation().getLatitude() != 0
			...
			for (Iterator iterator = ((java.util.List) results2).iterator(); iterator.hasNext();) {
				...
				if (CommonLib.isFacebookCheckValid) {
					...
					if (userResults != null && !userResults.isEmpty()) {
						...
						if (user != null) {
							shouldAdd = Friends.isFriendOnFacebook(user.getFacebookId(), currentUser.getFacebookId(),
	...
	public boolean setRatingForUser(int currentUser, int userId, double rating, int wishId, int userWishId) {
		...
		try {
			...
			/*
			 * { String sql =
			 * "SELECT count(*) FROM USERRATING WHERE Reviewed= :userId AND Reviewer= :userIdtwo"
			 * ; SQLQuery query = session.createSQLQuery(sql);
			 * query.setParameter("userId", userId);
			 * query.setParameter("userIdtwo", currentUser); java.util.List
			 * results = (java.util.List) query.list(); if (results.get(0)
			 * instanceof BigInteger) { int count = ((BigInteger)
			 * results.get(0)).intValue(); retVal = true; if (count == 0) {
			 * String sql2 =
			 * "INSERT INTO USERRATING(Reviewer,Reviewed,Rating) VALUES(:reviewer,:reviewed,:rating)"
			 ...
			 * query2.setParameter("rating", rating); query2.executeUpdate(); }
			 * else if (count == 1) { String sql2 =
			 * "Update USERRATING Set Rating = :rating where Reviewer = :reviewer and Reviewed = :reviewed"
			 * ; SQLQuery query2 = session.createSQLQuery(sql2);
			 * query2.addEntity(com.application.baatna.bean.UserRating.class);
			 * query2.setParameter("rating", rating);
			 * query2.setParameter("reviewer", currentUser);
			 * query2.setParameter("reviewed", userId); query2.executeUpdate();
			 * retVal = true; } } }
			 */
			transaction.commit();
			session.close();
	...
	/*
	 * public boolean editRatingForUser(int userIdtwo, int userId, double
	 * rating) { Session session = null; boolean retVal = false;
	 * 
	 * try {
	 * 
	 * session = DBUtil.getSessionFactory().openSession();
	 ...
	 * 
	 * String sql =
	 * "SELECT count(*) FROM USERRATING WHERE USERID_ONE= :userId AND USERID_TWO= :userIdtwo"
	 * ; // String sql = "UPDATE USERRATING SET Rating = :rating, WHERE //
	 * USERID = :user_id"; SQLQuery query = session.createSQLQuery(sql); //
	 * query.addEntity(com.application.baatna.bean.UserRating.class);
	 * query.setParameter("userId", userId); query.setParameter("userIdtwo",
	 * userIdtwo); java.util.List results = (java.util.List) query.list(); if
	 * (results.get(0) instanceof BigInteger) { int count = ((BigInteger)
	 * results.get(0)).intValue(); if (count != 0) { String sql2 =
	 * "UPDATE USERRATING SET RATING= :rating WHERE USERID_ONE= :userId AND USERID_TWO= :userIdtwo"
	 * ; SQLQuery query2 = session.createSQLQuery(sql2);
	 * query2.addEntity(com.application.baatna.bean.UserRating.class);
	 * query2.setParameter("userId", userId); query2.setParameter("userIdtwo",
	 * userIdtwo); query2.setParameter("rating", rating);
	 * query2.executeUpdate(); retVal = true; } } transaction.commit();
	 * session.close();
	 * 
